...
def countValidWords(sentence):
    ...
    for word in sentence_list:
        ...
        for ch in range(len(word)):
            if str(word).count('-') > 1:
                break
            elif word[ch] not in "abcdefghijklmnopqrstuvwxyz-!,.":
                break
            elif word[ch] == "-" and (ch== int(0) or ch== int(len(word)-1)): #Rule2
                break
            elif word[ch] == "-" and ((word[ch-1] not in "abcdefghijklmnopqrstuvwxyz") or (word[ch+1] not in "abcdefghijklmnopqrstuvwxyz")):
                break
            elif word[ch] in "!,." and (ch != len(word)-1):
                break
        else:
            output.append(word)
    return len(output)
