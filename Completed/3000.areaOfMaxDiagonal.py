...
def areaOfMaxDiagonal(dimensions):
    ...
    for dimension in dimensions:
        temp = math.sqrt((dimension[0]*dimension[0])+(dimension[1]*dimension[1]))
        if temp not in diagonal_length:
            diagonal_length[temp] = [(dimension[0]*dimension[1])]
        else:
            diagonal_length[temp] += [(dimension[0]*dimension[1])]
    return max(diagonal_length[max(diagonal_length)])


# dimensions = [[9,3],[8,6],[9,3],[8,6]]
